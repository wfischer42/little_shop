...
describe 'Merchant dashboard' do
  ...
  it "merchant visits dashboard and goes to orders" do
    ...
    expect(page).to have_content("Created At: #{ @order_1.created_at.strftime("%m-%d-%Y")}")
    expect(page).to have_content("Updated At: #{@order_1.updated_at.strftime("%m-%d-%Y")}")
    expect(page).to have_content("Order Status: #{@order_1.status}")
    expect(page).to have_content("Order Id: #{@order_2.id}")
    expect(page).to have_content("Created At: #{@order_2.created_at.strftime("%m-%d-%Y")}")
    expect(page).to have_content("Updated At: #{@order_2.updated_at.strftime("%m-%d-%Y")}")
    expect(page).to have_content("Order Status: #{@order_2.status}")
    expect(page).to have_content("Order Id: #{@order_4.id}")
    expect(page).to have_content("Created At: #{@order_4.created_at.strftime("%m-%d-%Y")}")
    expect(page).to have_content("Updated At: #{@order_4.updated_at.strftime("%m-%d-%Y")}")
